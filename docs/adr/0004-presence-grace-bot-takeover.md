...
5. **End game:** host may `EndGame` (WS) or `POST .../end`. Vacancy older than
   **10 minutes** auto-ends (`aborted`). Abort removes the actor from
   `state.games`, compacts the game row, and returns the room to **Lobby**
   (vacant seats dropped). Aborted rows are hard-deleted after 24h (reaper).
...
11. **Abandonment GC:** lobby TTL 1h; finished/aborted games purged after 24h;
...
- Abort/restart free `MAX_ACTIVE_GAMES` slots immediately (no 24h actor leak).
