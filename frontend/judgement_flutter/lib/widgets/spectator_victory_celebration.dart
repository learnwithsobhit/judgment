...
/// Audience end-game party beat before standings (no player-only actions).
class SpectatorVictoryCelebration extends StatefulWidget {
  final SpectatorController controller;
  final VoidCallback onViewResults;

  const SpectatorVictoryCelebration({
    super.key,
  ...
  @override
  State<SpectatorVictoryCelebration> createState() =>
      _SpectatorVictoryCelebrationState();
}

class _SpectatorVictoryCelebrationState
    extends State<SpectatorVictoryCelebration> with TickerProviderStateMixin {
    ...
    final seats = [
      for (final s in view.seats)
        (
          ...
          name: s.nickname,
          avatar: s.avatarId,
          winner: winnerIds.contains(s.playerId),
        ),
    ]..sort((a, b) {
        final ra = ranking
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    'Thanks for watching — crackers up, then check the scores',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      fontSize: 14,
